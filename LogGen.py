...
def Convert_to_Accdb(path, newpath):
    ...
    if os.path.isfile(path + newpath + r'\db12020.accdb'):
        print("File exists.")
    else:
        shutil.copyfile(path + r'\db12020.bds', path + newpath + r'\db12020.bds')
        p = Path(path + newpath + r'\db12020.bds')
...
def Connect_to_Database():
    ...
    MDB = path + newpath + r'\db12020.accdb'
...
for row in cur.tables():
    ...
    if row.table_type == 'TABLE':
        ...
        df.to_csv(r'CSV2\{}.csv'.format(row.table_name), index=False)
