...
# convert df to json
df = df.to_json("../JSONdata/Tran2.json" ,indent=4, orient="records")

# get data from json file
with open("../JSONdata/Tran2.json", "r") as f:
    data = json.load(f)
...
for row in data:
    ...
    # try catch block
    try:
        response = supabase.from_("tran2").insert(row).execute()
        print(response)
    except:
        print("Error")
