...
df = pd.read_csv("../CSV/Company.csv")
...
# convert df to json
df = df.to_json("../JSONdata/Company.json" ,indent=4, orient="records")

# get data from json file
with open("../JSONdata/Company.json", "r") as f:
    data = json.load(f)
...
for row in data:
    ...
    response = supabase.from_("company").insert(row).execute()
    print(response)
