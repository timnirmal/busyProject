...
def Sales_Order(id: int, tran1ColumnList=None, master1ColumnList=None, tran2ColumnList=None,
    ...
    ** Now we have All the data needed.
    ...
    for index, row in dfTran2.iterrows():
        # if rectype = 3 then srno = 0
        if row.rectype == 3:
            dfTran2.at[index, 'srno'] = 0

    # sort dfTran2 by itemname and srno
    dfTran2.sort_values(by=['vchno', 'itemname', 'srno'], inplace=True)
    ...
    # save df to csv
    dfTran2.to_csv("Sales_Order_New_Tran2_Sorted.csv", index=False)

    # find first row of each vchno to new df
    dfTran2FirstRow = dfTran2.groupby('vchno').first().reset_index()

    # for each row in dfTran2FirstRow, add name, mastertype, mastercode2 to dfTran2
    for index, row in dfTran2FirstRow.iterrows():
        vchno = row.vchno
        name = row.itemname
        mastertype = row.mastertypeitem
        mastercode2 = row.mastercode2
        dfTran2.loc[dfTran2['vchno'] == vchno, 'name'] = name
        dfTran2.loc[dfTran2['vchno'] == vchno, 'mastertype'] = mastertype
        dfTran2.loc[dfTran2['vchno'] == vchno, 'mastercode2'] = mastercode2

    print("After adding name, mastertype, mastercode2 to dfTran2")
    print("After adding name, mastertype, mastercode2 to dfTran2")
    print("After adding name, mastertype, mastercode2 to dfTran2")
    ...
    # save df to csv
    dfTran2.to_csv("Sales_Order_New_Tran2_Sorted_FirstRow.csv", index=False)
    ...
    for index, row in dfTran2.iterrows():
        ...
        # insert the name in dfTran2 at index
        dfTran2.at[index, 'name'] = dfTemp['name'].iloc[0]
        dfTran2.at[index, 'mastertype'] = dfTemp['mastertype'].iloc[0]
        dfTran2.at[index, 'date'] = dfTemp['date'].iloc[0]

        # from dfTemp get name
    ...
    dfTran2.to_csv("Sales_Order_Final_Tran2.csv", index=False)
